    ...
    <div className="container">
      ...
      {items?.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: '40px 24px' }}>
          ...
          <h2 style={{ marginTop: 8 }}>No products yet</h2>
          <p className="muted" style={{ maxWidth: 420, margin: '8px auto 0' }}>
            Add your first product to see whether it's in scope, its CRA risk class, and a
            step-by-step path to compliance.
          </p>
          <div className="btn-row" style={{ justifyContent: 'center', marginTop: 16 }}>
            <button className="btn" onClick={() => navigate('/')}>+ Classify your first product</button>
          </div>
        </div>
      )}
      {items?.map((a) => (
