...
export default function Admin() {
  ...
  return (
    <div className="container wide">
      ...
      <p className="muted">Manage accounts and review incoming requests. Signed in as {user?.email}.</p>
...
function RequestList({ listFn, onApprove, approveLabel = 'Approve → create', statusFn, deleteFn, kindLabel }) {
  ...
  async function approve(id) {
    setError(''); setCreated(null); setLink('');
    try {
      const d = await onApprove(id);
      if (d.demoUrl) setLink(d.demoUrl);
      else setCreated({ email: d.user.email, password: d.generatedPassword });
      refresh();
    } catch (err) { setError(err.message); }
  }
  ...
  return (
    <div className="card">
      ...
      {link && (
        <div className="card" style={{ background: '#0f172a', color: '#e2e8f0' }}>
          <strong>Demo approved — send this private link to the requester:</strong>
          <div className="small" style={{ marginTop: 8, fontFamily: 'monospace', wordBreak: 'break-all' }}>{link}</div>
          <div className="btn-row"><button className="btn" onClick={() => copy(link)}>Copy link</button></div>
          <p className="small" style={{ marginTop: 8, opacity: 0.8 }}>Only this link unlocks the read-only demo. It expires in 30 days.</p>
        </div>
      )}
      ...
      {items && items.map((it) => (
        <div key={it.id} className="list-item">
          <div className="grow">
            ...
            {it.status !== 'new' && <span className="badge" style={{ marginLeft: 8, background: '#1e293b', color: it.status === 'approved' ? 'var(--green)' : 'var(--muted)' }}>{it.status}</span>}
          ...
          {onApprove && <button className="btn" onClick={() => approve(it.id)}>{it.status === 'approved' ? 'Re-issue link' : approveLabel}</button>}
          {it.status === 'new'
...
function DemoTab() {
  return (
    <>
      <p className="muted small">People asking to see the live demo. "Approve → send link" generates a private demo link to email them; the read-only demo only opens with that link.</p>
      <RequestList
        kindLabel="demo requests"
        listFn={api.adminListDemoRequests}
        onApprove={api.adminApproveDemoRequest}
